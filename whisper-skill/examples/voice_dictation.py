...
def paste_from_clipboard() -> None:
    ...
    """
    # macOS: pynput (CGEvent) шлёт нажатие напрямую — под Терминалом с
    # Accessibility это реально доносит Cmd+V до поля. osascript→System Events
    # возвращает успех, но keystroke часто не доходит, поэтому он только fallback.
    if platform.system() == "Darwin":
        try:
            from pynput.keyboard import Controller, Key
            kb = Controller()
            # 1) Сбросить «залипший» Command от хоткея (хоткей сам на правом ⌘) —
            #    иначе состояние модификатора неоднозначно.
            for _k in (Key.cmd, Key.cmd_l, Key.cmd_r):
                try:
                    kb.release(_k)
                except Exception:
                    pass
            time.sleep(0.05)
            # 2) Чистый Cmd+V с явными паузами: без них V улетает раньше, чем
            #    система зарегистрирует Command-down, и вставка не срабатывает.
            kb.press(Key.cmd)
            time.sleep(0.05)
            kb.press("v")
            time.sleep(0.05)
        ...
        try:
            ...
            logging.error(
                f"osascript paste also failed: {e}\n"
...
def type_text_mac(text: str) -> bool:
    """Напечатать текст напрямую (юникод-события), без Cmd+V.

    Альтернатива вставке: на некоторых системах синтетический Cmd+V не доходит
    до поля (особенно когда хоткей сам на Command), а прямая печать символов
    через CGEventKeyboardSetUnicodeString проходит. Возвращает True при успехе.
    """
    try:
        from pynput.keyboard import Controller, Key
        kb = Controller()
        # сбросить «залипший» Command от хоткея
        for _k in (Key.cmd, Key.cmd_l, Key.cmd_r):
            try:
                kb.release(_k)
            except Exception:
        ...
        logging.error(f"type_text_mac failed: {e}; текст в clipboard — Cmd+V")
        return False
...
def main_loop(cfg: dict, cfg_path: Path):
            ...
            try:
                ...
                if not text:
                    ...
                    if cfg.get("auto_paste"):
                        ...
                        # На Mac печатаем напрямую (надёжнее, чем синтетический
                        # Cmd+V, который тут не доходит до поля). На остальных —
                        # обычная вставка из буфера.
                        if platform.system() == "Darwin":
                            type_text_mac(text_to_paste)
                        else:
                            paste_from_clipboard()
                    # Буфер НЕ восстанавливаем: распознанный текст остаётся в
                    # clipboard, чтобы Cmd+V всегда работал как запасной вариант,
                    # если авто-вставка по какой-то причине не сошлась.
    ...
    if cfg["mode"] == "ptt":
        ...
        # Сторона-aware: храним нажатые клавиши с суффиксом _l/_r, чтобы можно
        # было задавать конкретно правый/левый модификатор (напр. <cmd_r>).
        keys_needed = _parse_hotkey(hotkey_str)
        currently_pressed = set()

        def on_press(key):
            currently_pressed.add(_canonical_key_sided(key))
            if _ptt_satisfied(keys_needed, currently_pressed):
                start_recording()

        def on_release(key):
            ck = _canonical_key_sided(key)
            if state.is_recording and any(_key_satisfies(n, ck) for n in keys_needed):
                stop_and_transcribe()
...
def _canonical_key_sided(key) -> str:
    """Как _canonical_key, но СОХРАНЯЕТ суффикс _l/_r (ctrl_l, cmd_r, ...).

    Нужно для PTT с side-specific хоткеем вроде <cmd_r> (правый Command).
    """
    from pynput.keyboard import Key, KeyCode
    if isinstance(key, Key):
        return key.name  # ctrl_l / cmd_r / space / ...
    if isinstance(key, KeyCode):
        if key.char:
            return key.char.lower()
        return str(key)
...
def _key_satisfies(needed: str, pressed: str) -> bool:
    """needed из хоткея, pressed — реально нажатая (с суффиксом стороны).

    - needed со стороной (cmd_r) → совпадение только точное (правый Command).
    - needed без стороны (cmd / ctrl / shift) → матчит любую сторону.
    """
    if needed == pressed:
        return True
    return pressed in (needed + "_l", needed + "_r")


def _ptt_satisfied(keys_needed: set, currently_pressed: set) -> bool:
    return all(
        any(_key_satisfies(n, p) for p in currently_pressed)
